const events = [{
        ...
        image: "https://wallpaperaccess.com/full/2185816.jpg",
    },
    {
        title: "Twist a Tale",
        content: "Everyone has a story to tell, but with the stories, comes unpredictable twists and turns. On this note, Twist a Tale welcomes you into the world of tales where you need to bend your storylines to the will of the prophets.\nPrepare for a challenge, hold up your pens and be ready to exhibit your ingenious imagination while you artfully endeavour to incorporate the unexpected twists in your tale. Be ready to experience the magical journey of words, metaphors, emotions and…TWISTS!",
        short: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Omnis dolorem laudantium, sunt repudiandae eos sit exercitationem ipsa, velit sint quisquam quod soluta voluptas in consequuntur quas accusamus debitis reprehenderit odit?",
        image: "https://wallpaperaccess.com/full/2185816.jpg",
    },
    {
        title: "Poetry Slam",
        content: "Poetry is introspection, destruction and renaissance of words and emotions. Voice is a shuttle to convey those emotions to the audience. Together, they open a window to the poet's soul and give us a glimpse of their encounter.\nCreate a composition with your vibrantly coloured words, give life to those words with your artistic voice and paint a masterpiece to be heard by the masses.",
        short: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Omnis dolorem laudantium, sunt repudiandae eos sit exercitationem ipsa, velit sint quisquam quod soluta voluptas in consequuntur quas accusamus debitis reprehenderit odit?",
        image: "https://wallpaperaccess.com/full/2185816.jpg",
    },
    {
...
const card = document.querySelector(".eventInfo");
...
function clearGlow() {
    for (let img of eventImgs) {
        if (img.classList.contains("event-active")) {
            img.classList.remove("event-active");
        }
    }
    if (paidEvent.classList.contains("event-active")) {
        paidEvent.classList.remove("event-active");
    }
}

eventImgs[0].classList.add("event-active");

for (let i = 0; i < 12; i++) {
    eventImgs[i].style.content = `url(${events[i].image})`;
    eventImgs[i].addEventListener("click", (e) => {
        clearGlow();
        eventImgs[i].classList.add("event-active");
        card.style.animation = "disappear 1s forwards";
        setTimeout(() => {
...
paidEvent.style.backgroundImage = `url(${events[12].image})`;

paidEvent.addEventListener("click", (e) => {
    clearGlow();
    paidEvent.classList.add("event-active");
    card.style.animation = "disappear 1s forwards";
    setTimeout(() => {
